...
module.exports = {
  HTML:function(itemname, list, body, control){
    ...
    <div class="jumbotron text-center"">
      ...
      <a class="nav-link active" href="/addLook">ADD</a>
      <br><br>
    ...
    `;
  },list:function(items,userlookname){
    var list = '<ul class="list-group list-group-flush">';
    var i = 0;
    while(i < items.length){
      list = list + `
      <h3>&nbsp&nbsp${userlookname}</h3>
      <li class="list-group-item"><a class="nav-link active" href="/viewItems?id=${items[i].itemid}">${(items[i].itemname)} : ${(items[i].itemimg)}</a></li>`;
